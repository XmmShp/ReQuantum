...
public class PtaProblemSetService : IPtaProblemSetService
{
    ...
    public async Task<Result<List<PtaProblemSet>>> GetProblemSetsAsync()
    {
        ...
        if (!clientResult.IsSuccess)
            return Result<List<PtaProblemSet>>.Failure(clientResult.Message);
        ...
        catch (Exception ex)
        {
            ...
            return Result<List<PtaProblemSet>>.Failure($"获取习题集异常: {ex.Message}");
        }
    }
}
